...
const FontForm = ({font_id}: Props) => {
    ...
    useEffect(() => {
        ...
        setFormData({
            ...
            imageFont: {
                title: imageFont.title,
                unit: imageFont.unit,
                detailMob: imageFont.detail_mobile,
                detailPc: imageFont.detail_pc
            },
        });
    }, [data]);
            ...
            <Form isGrey={true} onSubmit={handleSubmit}>
                <Grid gap={`36px 22px`} padding={`36px 24px`}>
                    ...
                    <GridLayout>
                        ...
                        <InputText id="corporation" placeholder="text" value={formData.corporation}
                                   onChange={handleTextChange}/>
                    </GridLayout>
                    <GridLayout row={`1 / span 2`} column={`2 / span 1`}>
                        <label htmlFor="description">Memo</label>
                        <InputTextArea id="description" placeholder="text" value={formData.description}
                                       onChange={handleTextChange}/>
                    </GridLayout>
                </Grid>
                <GridDivider/>
                <Grid gap={`36px 22px`} padding={`36px 24px`}>
                    <GridLayout column={`span 2`}>
                        ...
                        <Radio>
                            <input type="radio" name="isWebFont" id="webFont" value="true" checked={formData.isWebFont}
                                   onClick={() => handleIsWebFont(true)}/>
                            <label htmlFor="webFont">웹 폰트</label>
                        </Radio>
                        <Radio>
                            <input type="radio" name="isWebFont" id="imageFont" value="false"
                                   checked={!formData.isWebFont} onClick={() => handleIsWebFont(false)}/>
                ...
                <GridDivider/>
                {formData.isWebFont ? (
                    <WebFont data={formData.webFont}
                             onSubmit={(data) => handleFormData({...formData, isWebFont: true, webFont: data})}/>
                ) : (
                    <ImageFont data={formData.imageFont}
                               onSubmit={(data) => handleFormData({...formData, isWebFont: false, imageFont: data})}/>
                )}
                <GridDivider/>
