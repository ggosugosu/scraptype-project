...
import {
  ...
  GET_TAG_ALL,
  UPDATE_TAG,
} from 'features/Admin/Tag/Overview/gql';
import {
  ButtonNegativeStyle,
  ButtonPositiveStyle,
  NewTagItemStyle,
...
const TagItem = (item: TagItemProps) => {
  ...
  const handleModifyCompleted = useCallback(
    async (e) => {
      e.preventDefault();
  ...
  const handleDelete = useCallback(async () => {
    const confirmed = confirm(
      '정말 삭제하시겠습니까? 기존에 연결되어있던 태그들은 모두 해제됩니다.'
    );
    if (!confirmed) return;
    ...
    <TagItemWrapperStyle>
      {isOpen ? (
        <TagItemStyle
          as={'form'}
          key={item.id}
          onSubmit={handleModifyCompleted}
        >
          <label htmlFor={'input-tag-name'} className={'hidden'}>
            {modifiedTagName}의 태그 이름을 수정하세요.
          </label>
          <InputText
            id={'input-tag-name'}
            ...
            height={'36px'}
          />
          <ButtonPositiveStyle onClick={handleModifyCompleted} type={'submit'}>
            완료
          </ButtonPositiveStyle>
          <ButtonNegativeStyle onClick={handleCancel} type={'button'}>
            취소
          </ButtonNegativeStyle>
        </TagItemStyle>
      ) : (
        <TagItemStyle key={item.id}>
          <div>
            {modifiedTagName}({item.fontTags.length})
          </div>
          <ButtonPositiveStyle onClick={handleShowModifyView}>
            수정
          </ButtonPositiveStyle>
