...
import { GET_FONTS_BY_TAG_ID, GET_TAGS_BY_TAG_ID } from "features/Search/SearchResult/gql";
import { TagsWrapper, ResultsWrapper } from "features/Search/SearchResult/style";
...
const TagContainer = ({keywords}: Props) => {
  ...
  return <TagsWrapper>{data && data.getTagsByTagId.map((tag) => <HighlightButton key={tag.id} name={tag.name}
                                                                                 selected={true}/>)}</TagsWrapper>;
};
